...
mod tests {
    ...
    // node's `POM_TIERS_H4` order per the block's own DAA, and MUST be None below the flip.
    const PRE_H4: u64 = crate::pom::COIN_AGE_VERIFICATION_ACTIVATION_DAA - 1;
    const AT_H4: u64 = crate::pom::COIN_AGE_VERIFICATION_ACTIVATION_DAA;
    ...
    #[test]
    fn pom_tier_index_at_h4_matches_node_order() {
        assert_eq!(pom_tier_index(&EXAONE_4_0_1_2B.model_id, AT_H4), Some(0));
        assert_eq!(pom_tier_index(&MISTRAL_7B_V03.model_id, AT_H4), Some(1));
        assert_eq!(pom_tier_index(&GLM_4_9B_0414.model_id, AT_H4), Some(2));
        assert_eq!(pom_tier_index(&QWEN3_6_27B.model_id, AT_H4), Some(3));
        assert_eq!(pom_tier_index(&KIMI_LINEAR_48B.model_id, AT_H4), Some(4));
    }

    #[test]
    fn every_h4_model_has_a_pinned_anchor() {
        let expected_chunks =
            [28_943_588u64, 185_827_840, 258_040_832, 516_762_688, 927_994_064];
